-- 商品双价格：店铺下单价 (price) + 一般顾客价 (public_price)
-- public_price 为 NULL 时与 price 相同
-- 在 Supabase SQL Editor 执行（可重复执行）

ALTER TABLE products ADD COLUMN IF NOT EXISTS public_price NUMERIC;

CREATE OR REPLACE FUNCTION fos_product_public_unit_price(p products)
RETURNS NUMERIC
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT coalesce(p.public_price, p.price);
$$;

-- 顾客端商品列表：price = 顾客实付单价，shop_price = 店铺价（管理参考）
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'products' AND column_name = 'public_price'
  ) INTO v_has_public_price;

  IF v_has_sort_order AND v_has_public_price THEN
    SELECT coalesce(
    ...
    FROM (
      SELECT
        id, name, name_zh, spec,
        coalesce(public_price, price) AS price,
        price AS shop_price,
        public_price,
        tax_rate, category, emoji, image_url, stock, active,
        sort_order
      FROM products
      WHERE merchant_id = v_ch.merchant_id AND active = TRUE
      ORDER BY sort_order NULLS LAST, category NULLS LAST, name, id
    ) p;
  ELSIF v_has_sort_order THEN
    SELECT coalesce(
      jsonb_agg(row_to_json(p) ORDER BY p.sort_order NULLS LAST, p.category NULLS LAST, p.name, p.id),
      '[]'::jsonb
    )
...
-- 散客下单：按 public_price（缺省同 price）计价
...
GRANT EXECUTE ON FUNCTION fos_product_public_unit_price(products) TO anon, authenticated;
