-- 热修复：create_public_order · uuid = text
-- 根因：products.id 为 UUID，payload 中 product_id 为 TEXT
--       WHERE id = trim(v_item->>'product_id') 触发 operator does not exist: uuid = text
